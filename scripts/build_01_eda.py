"""Gera notebooks/01_eda.ipynb de forma programática (nbformat).

Mantém o .ipynb reprodutível e fácil de versionar (outputs são removidos pelo
nbstripout no commit). Rode: python scripts/build_01_eda.py
"""
...
cells.append(md(
    ...
    "\n"
    "Inspeção das 4 séries da B3 (PETR4, VALE3, AMER3, ITUB4): integridade, gaps, "
    "NaNs, dias sem pregão, splits/grupamentos e anomalias reais conhecidas.\n"
    "\n"
    "A lógica vive em `src/`; este notebook só orquestra e visualiza. Dados lidos do "
    "cache `data/raw/` (offline) — ver `src/data.py` e `docs/adr/0007-coleta-e-tratamento-amer3.md`."
))
...
cells.append(md(
    ...
    "Diferença entre dias úteis (business days) do calendário e pregões efetivos "
    "revela feriados/halts. Não há feriados da B3 embutidos aqui; serve só de panorama."
))
cells.append(code(
...
cells.append(md(
    ...
    "- **12/01/2023** — divulgação da fraude contábil; queda de ~78% no dia (log-ret ≈ -1.48). "
    "Anomalia **real** que o modelo deve detectar — não limpar.\n"
    "- **14/11/2024** — salto de log-ret ≈ +1.03, compatível com **grupamento (reverse split)** "
    "não totalmente ajustado pelo yfinance — possível **artefato** a tratar.\n"
...
cells.append(md(
    ...
    "- Anomalias reais identificadas como âncoras narrativas (M6): PETR4 03/2020 (COVID), "
    "VALE3 01/2019 (Brumadinho), AMER3 01/2023 (fraude), ITUB4 10/2021.\n"
